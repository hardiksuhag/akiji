...
var maxprobdata = document.querySelector("#maxprobdata");
...
function refresh(answer){
	...
	var flag3=(Number(questions_asked.length)>=15);
    ...
    animate();
	question.textContent=('Q-'+Number(questions_asked.length)+') '+questions[new_question_pk].question_text);
}
...
yes.addEventListener("click",function(){
    setTimeout(refresh_1,100);
});
idk.addEventListener("click",function(){
    setTimeout(refresh_0,100);
});
no.addEventListener("click",function(){
    setTimeout(refresh_m1,100);
});
...
idk.addEventListener("mouseenter",function(){
    this.style.opacity=0.7;
});
idk.addEventListener("mouseleave",function(){
    this.style.opacity=1;
})
